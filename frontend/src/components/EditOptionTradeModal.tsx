...
import { useUpdateOptionTrade } from '../hooks/useUpdateOptionTrade'
import type { OptionTrade, OptionTradeUpdate } from '../api/types'
...
export default function EditOptionTradeModal({ trade, onClose }: Props) {
  const mutation = useUpdateOptionTrade()
  ...
  const [form, setForm] = useState({
    ...
    open_commission: String(trade.open_commission),
    status: trade.status,
    exit_date: trade.exit_date ?? '',
    exit_price: trade.exit_price != null ? String(trade.exit_price) : '',
    close_commission: String(trade.close_commission),
    notes: trade.notes ?? '',
  })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    mutation.mutate({ id: trade.id, body }, { onSuccess: onClose })
  }
  ...
  return (
    <Modal title={`Edit Option Trade — ${trade.underlying_symbol}`} onClose={onClose}>
      <form onSubmit={handleSubmit} className="space-y-4">
        {/* Underlying symbol (read-only) */}
        <div>
